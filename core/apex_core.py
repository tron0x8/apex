...
from .php_parser import parse_php, ASTNode, NodeType
...
class VulnerabilityReport:
    ...
    remediation: str = ""
    cwe: str = ""
    ml_verified: bool = False           # ML verification status
    ml_confidence: float = 0.0          # ML model confidence
    ml_label: str = ""                  # ML prediction label
    filtered_by_ml: bool = False        # Was this filtered as FP?
...
class APEXCore:
    ...
    def __init__(self, workers: int = 4, verbose: bool = False,
                 ml_filter: BaseMLFilter = None, ml_model_path: str = None):
        self.workers = workers
        self.verbose = verbose
        self.findings: List[VulnerabilityReport] = []
        self.filtered_findings: List[VulnerabilityReport] = []  # FP filtered by ML
        self.stats = {
            'files_scanned': 0,
        ...
        # ML Filter setup
        self.ml_filter = ml_filter or NoOpFilter()
        if ml_model_path and not isinstance(self.ml_filter, NoOpFilter):
            if not self.ml_filter.load_model(ml_model_path):
                if self.verbose:
                    print(f"[!] Failed to load ML model: {ml_model_path}")
                self.ml_filter = NoOpFilter()
        elif ml_model_path:
            # Try to load default PHP-vuln-ml filter
            try:
                from .ml_filter import PHPVulnMLFilter
                self.ml_filter = PHPVulnMLFilter()
                if self.ml_filter.load_model(ml_model_path):
                    if self.verbose:
                        print(f"[+] ML filter loaded: {ml_model_path}")
                else:
                    self.ml_filter = NoOpFilter()
            except ImportError:
                self.ml_filter = NoOpFilter()
    ...
    def analyze(self, target: str, mode: str = 'full') -> List[VulnerabilityReport]:
        ...
        # ML Filtering Phase
        if self.ml_filter.is_loaded and not isinstance(self.ml_filter, NoOpFilter):
            if self.verbose:
                print("[*] Phase 4: ML False Positive Filtering")
            self._apply_ml_filter()
    ...
    def _apply_ml_filter(self):
        """Apply ML filter to remove false positives"""
        verified_findings = []

        for finding in self.findings:
            # Get code context for ML analysis
            code_context = self._get_code_context(finding.file, finding.line)

            # Run ML prediction
            result = self.ml_filter.predict(finding.to_dict(), code_context)

            if result.is_false_positive:
                # Mark as filtered and move to filtered list
                finding.filtered_by_ml = True
                finding.ml_confidence = result.confidence
                finding.ml_label = result.ml_label
                self.filtered_findings.append(finding)
                self.stats['ml_filtered'] += 1

                if self.verbose:
                    print(f"    [FP] {finding.file}:{finding.line} - {result.reason}")
            else:
                # Keep finding and mark as verified
                finding.ml_verified = True
                finding.ml_confidence = result.confidence
                finding.ml_label = result.ml_label
        ...
        if self.verbose:
            print(f"    Verified: {self.stats['ml_verified']}, Filtered: {self.stats['ml_filtered']}")

    def _get_code_context(self, file_path: str, line: int, context_lines: int = 20) -> str:
        """Get code context around a finding for ML analysis"""
        ...
        code = self._file_cache[file_path]
        lines = code.split('\n')
    ...
    def generate_report(self, output_format: str = 'json', include_filtered: bool = False) -> str:
        if output_format == 'json':
            report = {
                'summary': {
                    ...
                    'ml_verified': self.stats.get('ml_verified', 0),
                    'ml_filtered': self.stats.get('ml_filtered', 0),
                },
                'stats': self.stats,
                'ml_filter': self.ml_filter.get_model_info() if hasattr(self.ml_filter, 'get_model_info') else {},
                'findings': [f.to_dict() for f in self.findings]
            }

            if include_filtered:
                report['filtered_findings'] = [f.to_dict() for f in self.filtered_findings]
    ...
    def _generate_text_report(self) -> str:
        ...
        # ML stats
        if self.stats.get('ml_verified') or self.stats.get('ml_filtered'):
            lines.extend([
                "",
                "ML FILTERING",
                "-" * 70,
                f"ML Verified: {self.stats.get('ml_verified', 0)}",
                f"ML Filtered (FP): {self.stats.get('ml_filtered', 0)}",
            ])
...
def main():
    ...
    parser.add_argument('--ml-filter', type=str, default=None,
                       help='ML filter type: php-vuln-ml, taint-flow, or custom')
    parser.add_argument('--ml-model', type=str, default=None,
                       help='Path to ML model file (.pkl)')
    parser.add_argument('--show-filtered', action='store_true',
                       help='Include ML-filtered findings in report')
    ...
    |  /_/   \_\_|   |_____/_/\_\                                               |
    |                                                                           |
    |  Advanced PHP Exploitation Scanner + ML                                   |
    ...
    if args.ml_filter:
        try:
            ml_filter = MLFilterRegistry.create(args.ml_filter, args.ml_model)
            if args.verbose:
                print(f"[+] ML Filter: {args.ml_filter}")
        except Exception as e:
            print(f"[!] Failed to load ML filter: {e}")
    ...
    report = analyzer.generate_report(args.format, include_filtered=args.show_filtered)
    ...
    if analyzer.stats.get('ml_filtered'):
        print(f"    ML Verified: {analyzer.stats['ml_verified']}")
        print(f"    ML Filtered: {analyzer.stats['ml_filtered']}")
