...
@dataclass
class PropagationDef:
    name: str
    propagates_from: List[int] = field(default_factory=list)
...
# loads all yaml rule definitions from rules/ directory
class RuleEngine:
    ...
    def __init__(self, rules_dir: Optional[str] = None):
        ...
        self.propagators: Dict[str, PropagationDef] = {}
        self._load_all()
    ...
    def _load_propagators(self):
        data = self._load_yaml('builtin_sigs.yml')
        for entry in data.get('propagators', []):
            if not isinstance(entry, dict):
                continue
            name = entry.get('name', '')
            if name:
                self.propagators[name] = PropagationDef(
                    name=name,
                    propagates_from=entry.get('propagates_from', []),
                    sanitizes=entry.get('sanitizes', []),
                )

    def get_propagator(self, func_name: str) -> Optional[PropagationDef]:
        return self.propagators.get(func_name)
